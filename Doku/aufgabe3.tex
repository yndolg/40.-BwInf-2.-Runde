...
\begin{align}
    f(p, e) = \begin{cases}
        ...
        \min_{x\in [0, 15]} f(p + 1, e + n(w_p, x)) + d(w_p, x) &\text{, anderfalls}
...
\label[]{sec:prove}
\subsection{Algorithmus \ref*{alg:precalc}}
...
Folglich ist $dp[p][e]$ nach allen $i = n$ Iterationen für alle $1 \leq p \leq n+1 $, und damit Algorithmus \ref*{alg:precalc}, korrekt.  
